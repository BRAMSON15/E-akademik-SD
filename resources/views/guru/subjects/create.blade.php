...
<div class="card-container">
    <h3 style="font-weight: 600; margin-bottom: 1rem; border-bottom: 2px solid var(--gray-light); padding-bottom: 0.5rem;">Form Tambah Mata Pelajaran</h3>
    ...
    <form action="{{ route('guru.subjects.store') }}" method="POST">
        ...
        <div style="margin-bottom: 1rem;">
            ...
            <input type="text" name="name" value="{{ old('name') }}" required style="width: 100%; padding: 0.5rem; border: 1px solid var(--gray-light); border-radius: 0.25rem;" placeholder="Contoh: Matematika, Bahasa Indonesia, dll">
        </div>
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <select name="class" required style="width: 100%; padding: 0.5rem; border: 1px solid var(--gray-light); border-radius: 0.25rem;">
                ...
                @foreach($classes as $class)
                    <option value="{{ $class }}" {{ old('class') == $class ? 'selected' : '' }}>Kelas {{ $class }}</option>
                @endforeach
            </select>
        </div>
